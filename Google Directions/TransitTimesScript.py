...
texta = 'OID,Raw Input,City,State,Zip,Distance to Cranford in miles,Time to Cranford in minutes'
texta = texta + '\n'
...
with open('C:/TravelTimes/EmployeesF.csv') as csvfile:
    ...
    for row in reader:
        tempAr = []
##        tempAr.append(str(row['Employee Residence']))

        zips.append(row['Raw Input'])

##        if (row['Employee Residence'] not in zips):
##            zips.append(row['Employee Residence'])


destinations = ['40.655702, -74.303248']
